...
public class MergeSort {
    ...
    private static <Item extends Comparable> Queue<Queue<Item>>
        ...
        Queue<Queue<Item>> resultQueues = new Queue<>();
        for (Item item: items) {
            Queue<Item> nextQueue = new Queue<>();
            nextQueue.enqueue(item);
            resultQueues.enqueue(nextQueue);
        }
        return resultQueues;
    }
    ...
    public static <Item extends Comparable> Queue<Item> mergeSort(
        ...
        Queue<Queue<Item>> resultQueues = makeSingleItemQueues(items);
        while (resultQueues.size() > 1) {
            Queue<Item> q1 = resultQueues.dequeue();
            Queue<Item> q2 = resultQueues.dequeue();
            Queue<Item> mergeSortedQueue = mergeSortedQueues(q1, q2);
            resultQueues.enqueue(mergeSortedQueue);
        }
        return resultQueues.dequeue();
    }
}
